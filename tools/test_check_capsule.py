...
Exit 0 when both hold. Standard library only, temporary files, nothing
...
def build_jet(root, defective):
    ...
    write(os.path.join(root, "summary.json"),
          '{\n  "case": "jet_r2_re100",\n'
          '  "reference": {"length_scale_D_m": 2.0}%s\n}\n' % trailing)
...
def build_bad_name(root):
    """Wrong root name, and reference scales shipped as bare numbers."""
...
def main():
    ...
    try:
        ...
        got = failed_checks(jet_ok)
        if got:
            failures.append("corrected jet should pass, failed %s"
                            % sorted(got))
        else:
            print("PASS  jet after correction passes every check")

        # 2. Coverage.
        fired = set()
        for root in (jet_bad, torture, anchorless, badname):
